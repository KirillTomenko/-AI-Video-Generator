...
from config import VIDEO_MODELS, CHEAPEST_FOR_SHORT
...
def duration_keyboard(model_key: str) -> InlineKeyboardMarkup:
    ...
    mn, mx = cfg["min_duration"], min(cfg["max_duration"], 5)
    buttons = []
    row = []
    for d in range(mn, mx + 1):
        cost = cfg["price_per_sec"] * d
        row.append(InlineKeyboardButton(f"{d}с (~${cost:.3f})", callback_data=f"dur:{d}"))
        if len(row) == 3:
            buttons.append(row)
            row = []
    if row:
...
def format_models_text() -> str:
    ...
    for key, cfg in VIDEO_MODELS.items():
        ...
        lines.append(
            ...
            f"  ⏱ Диапазон: {cfg['min_duration']}–{cfg['max_duration']} сек\n"
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "💡 *Совет:* для 2-3 сек роликов используйте `ltx-video-fast` — "
...
async def cmd_generate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🎬 *Выберите модель:*\n\n"
        "_Для 2-3 сек роликов LTX Fast — самая дешёвая._",
...
async def handle_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    model_key = session.get("model", CHEAPEST_FOR_SHORT)
    duration  = session.get("duration", VIDEO_MODELS[model_key]["min_duration"])
    ...
    # Запускаем генерацию в фоновом потоке (не блокируем event loop)
    def run_generation():
        import asyncio
        ...
        def on_progress(status):
            ...
            asyncio.run_coroutine_threadsafe(
                bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=msg_id,
                    text=f"⏳ Статус: `{status}` (задача #{task_id[:8]}...)",
                    parse_mode="Markdown",
                ),
                asyncio.get_event_loop()
            )

        result = wait_for_completion(task_id, timeout=300, poll_interval=5)
